...
export const Post: FunctionComponent<Props> = props => {
    ...
    const comments = useSelector((state: RootState) => state.posts.commentsById[id]);
    ...
    return (
        <div className="post-container" >
            ...
            {comments && !showMore && <Comment comment={comments[comments.length-1]}/>}
           
            <div className="list-comments" data-testid="list-comment" >
                {comments && comments.length > 1 && showMore && 
            ...
            {comments && comments.length>1 && <Button onClick={() => {
                                setShowMore(!showMore)
                }} color="link">{showMore?'Show less':'Show more' }</Button>}
            </div>
